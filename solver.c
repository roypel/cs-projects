...
void builder(board *buildBoard, int x, int y) {
	...
	if (y != buildBoard->rows * buildBoard->cols) {
		posValues = calloc(buildBoard->rows * buildBoard->cols + 1, sizeof(int));/*Possible values for cell <x,y>*/
		...
		while (posValues[0] != 0) {/*There are still possible values to insert to cell <x,y>*/
			...
			if (buildBoard->board[buildBoard->rows * buildBoard->cols - 1][buildBoard->rows * buildBoard->cols - 1]->value != 0)/*Board is full, no need for further tries*/
				break;
			buildBoard->board[x][y]->value= 0;
		}
		printf("for row %d column %d, posValues are: ", y,x);
		for (i = 0; i< buildBoard->rows*buildBoard->cols+1; i++){
...
void hinter(gameState *metaBoard) {
	...
	printf("%d", metaBoard->filledCells);
